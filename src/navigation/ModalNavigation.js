...
export default function ModalNavigation(props) {
    ...
    const buttonClose = () => {
        return <IconButton icon={"window-close"} style={{ marginLeft: 15 }} size={24} onPress={() => props.navigation.goBack()} />;
    };

    const pdfButtonClose = () => {
        return <IconButton icon={"window-close"} style={{ marginLeft: 5, marginTop: -10 }} size={24} onPress={() => props.navigation.goBack()} />;
    };

    const buttonCloseLight = () => {
        return <IconButton icon={"window-close"} iconColor={"#fff"} style={{ marginLeft: 15 }} size={24} onPress={() => props.navigation.goBack()} />;
    };

    const buttonCloseColor = () => {
        return <IconButton icon={"window-close"} iconColor={ColorsApp.PRIMARY} style={{ marginLeft: 15 }} size={24} onPress={() => props.navigation.goBack()} />;
    };

    const buttonBack = () => {
        return <IconButton icon={I18nManager.isRTL ? "arrow-right" : "arrow-left"} style={{ marginLeft: 15 }} size={24} onPress={() => props.navigation.goBack()} />;
    };

    const buttonBackToHome = () => {
        return <IconButton icon={I18nManager.isRTL ? "arrow-right" : "arrow-left"} style={{ marginLeft: 15 }} size={24} onPress={() => props.navigation.navigate("home")} />;
    };
    ...
    return (
        ...
        >
            ...
            <RootStack.Screen name="exercisedetails" component={ExerciseDetails} options={{ title: headerTitle ? headerTitle : Strings.ST80, headerLeft: () => buttonClose() }} />
            ...
            <RootStack.Screen name="pdf" component={Pdf} options={{ headerTransparent: true, title: null, headerLeft: () => pdfButtonClose() }} />
