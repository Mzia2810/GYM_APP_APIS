...
export default function LatestWorkouts(props) {
  ...
  const onChangeScreen = (id, title, image) => {
    navigation.navigate('dietdetails', { id, title, image });
  };
    ...
    return (
      <View style={{ marginHorizontal: 10 }}>
        ...
        <FlatGrid
          ...
          data={data}
          renderItem={({ item }) => (
            <TouchableScale activeOpacity={1} onPress={() => onChangeScreen(item._id, item.title, item?.image)} activeScale={0.98} tension={100} friction={10}>
