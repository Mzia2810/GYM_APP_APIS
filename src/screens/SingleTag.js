...
import { categoryBlog } from '../apis/ApiHandlers';
...
export default function SingleTag(props) {
  ...
  console.log("🚀 ~ file: SingleTag.js:26 ~ SingleTag ~ title:", title)
  console.log("🚀 ~ file: SingleTag.js:26 ~ SingleTag ~ _id:", _id)
  const [page, setPage] = useState(1);
  const [isLoaded, setIsLoaded] = useState(false);
  const [data, setData] = useState([]);
  console.log("🚀 ~ file: SingleTag.js:32 ~ SingleTag ~ data:", data)
  ...
  useEffect(() => {
    ...
    //   setIsLoaded(true);
    // })
    categoryBlog(_id).then(response => {
      ...
      >
        ...
        <View style={Styles.ContentScreen}>
          ...
          {map(data, (item, i) => (
            ...
            <TouchableScale key={i} activeOpacity={1} onPress={() => onClickItem(item)} activeScale={0.98} tension={100} friction={10}>
              <ImageBackground source={{ uri: `${IMAGE_URL}/${item?.image}` }} style={Styles.card3_background} imageStyle={{ borderRadius: 8 }}>
