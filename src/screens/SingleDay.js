...
export default function SingleDay(props) {
    ...
    console.log("Day Single :::", day);
    const [isLoaded, setIsLoaded] = useState(false);
    const [items, setItems] = useState([]);
    const { setHeaderTitle } = useFavourite();
    ...
    const onClickItem = (id, title) => {
        ...
        // console.log('Item Pressed Title  ::  ',id,title);
    };
    ...
    useEffect(() => {
        getWorkoutByDay(id, day).then((response) => {
            setItems(response);
            setIsLoaded(true);
            // console.log("Single day screen useEffect call response ::  ", response);
        });
    }, []);
                            ...
                            {map(items, (item, i) => (
                                <TouchableScale key={i} activeOpacity={1} onPress={() => onClickItem(item.id, item.title)} activeScale={0.98} tension={100} friction={10}>
                                    <List.Item
                                        ...
                                        right={(props) => <List.Icon {...props} icon={rightIcon} style={{ opacity: 0.5, alignSelf: "center" }} />}
                                    />
                                </TouchableScale>
                            ))}
                        </SafeAreaView>
